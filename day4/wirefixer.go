...
func (p *point) manhattanDistanceTo(q *point) int64 {
	return abs(int64(q.x-p.x)) + abs(int64(p.y-q.y))
}
...
func getDelta(direction string) *point {
	...
	case "D":
		return NewPoint(0, -1)
	default:
		panic("Direction of " + direction + " is not valid")
	}
}
...
func parseMovement(str string) movement {
	mag, err := strconv.Atoi(str[1:])
	if err != nil {
		panic(err)
	}
	return movement{string(str[0]), mag}
}

func getMovements(rawInput string) []movement {
	var movements []movement
	for _, m := range strings.Split(rawInput, ",") {
		movements = append(movements, parseMovement(m))
	}

	return movements
}

type pathVisitor func(p point)

var origin *point = NewPoint(0, 0)

func traversePath(movements []movement, visit pathVisitor) {
	p := origin
	for _, m := range movements {
		delta := getDelta(m.direction)
...
func getMinDistanceToIntersection() int64 {
	// Solve the problem by saving a map of points visited by the first wire, then traversing the second wire and checking if the
	// first wire has already hit this path
	firstWireMovements := getMovements(firstWire)
	secondWireMovements := getMovements(secondWire)

	visited := make(map[point]bool)
	var intersections []point

	firstWireVisitor := func(p point) {
		visited[p] = true
	}
	secondWireVisitor := func(p point) {
		if visited[p] {
			intersections = append(intersections, p)
		}
	}

	traversePath(firstWireMovements, firstWireVisitor)
	traversePath(secondWireMovements, secondWireVisitor)

	var minDistance int64 = 9223372036854775807
	for _, p := range intersections {
		minDistance = min(minDistance, origin.manhattanDistanceTo(&p))
	}
...
func getMinStepsToIntersection() int64 {
	// Solve the problem by saving a map of points visited by the first wire (along with steps), then traversing the second wire and checking if the
	// first wire has hit the path while saving the steps needed. Then we iterate over all the intersections and calculate the minimum steps needed
	firstWireMovements := getMovements(firstWire)
	secondWireMovements := getMovements(secondWire)

	visited := make(map[point]int64)
	var intersections []pointWithSteps
	var steps int64 = 1
	...
	var secondWireSteps int64 = 1
	secondWireVisitor := func(p point) {
		firstWireSteps, beenVisited := visited[p]
		if beenVisited {
			q := pointWithSteps{p.x, p.y, secondWireSteps + firstWireSteps}
			intersections = append(intersections, q)
		}
		secondWireSteps++
	}
	...
	var minSteps int64 = 9223372036854775807
	for _, p := range intersections {
		minSteps = min(minSteps, p.steps)
	}
...
// Main driver
func main() {
	fmt.Printf("Minimum manhattan distance to closest intersection is %d\n", getMinDistanceToIntersection())
	fmt.Printf("Minimum steps to get to intersection is %d\n", getMinStepsToIntersection())
}
