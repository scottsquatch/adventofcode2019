...
func NewComputer(in <-chan int, out chan<- int) *Computer {
	return &Computer{in, out}
}

// Run an IntCode program with the current computer
func (comp Computer) Run(program *IntCodeProgram) *IntCodeProgram {
	intCodeCopy := make([]int, len(program.programData))
	copy(intCodeCopy, program.programData)
	currentProgram := IntCodeProgram{intCodeCopy}
	ip := 0
	halt := false
